    ...
    if (event === 'member-update') {
      removeListeners.push(
        addClientListener(client, 'guildMemberUpdate', async (oldMember, newMember) => {
          ...
          const oldRoleIds = [...oldMember.roles.cache.keys()].filter((id) => id !== newMember.guild.id);
          const newRoleIds = [...newMember.roles.cache.keys()].filter((id) => id !== newMember.guild.id);
          this.emit([
            this.helpers.returnJsonArray({
              type: 'member-update',
        ...
        addClientListener(client, 'guildScheduledEventCreate', async (scheduledEvent) => {
          ...
          this.emit([
            this.helpers.returnJsonArray({
              ...
              userCount: scheduledEvent.userCount ?? null,
        ...
        addClientListener(client, 'guildScheduledEventUpdate', async (oldEvent, newEvent) => {
          ...
          this.emit([
            this.helpers.returnJsonArray({
              ...
              oldScheduledStartTime: oldEvent?.scheduledStartAt ?? null,
              newScheduledStartTime: newEvent.scheduledStartAt,
              oldScheduledEndTime: oldEvent?.scheduledEndAt ?? null,
              newScheduledEndTime: newEvent.scheduledEndAt,
